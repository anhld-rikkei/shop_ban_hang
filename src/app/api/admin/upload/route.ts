 ...
 * Both are stored under uploads/products/<yyyy-mm>/ and the public URLs are returned.
 */
export async function POST(req: NextRequest) {
  ...
  const saved = await saveUpload(`products/${month}`, `${base}${ext}`, Buffer.from(await image.arrayBuffer()));
  let thumbUrl = saved.url;
  if (thumb instanceof File && IMAGE_MIMES.has(thumb.type) && thumb.size <= MAX_UPLOAD_BYTES) {
    const t = await saveUpload(`products/${month}`, `${base}-300x300${extForMime(thumb.type) || ".jpg"}`, Buffer.from(await thumb.arrayBuffer()));
    thumbUrl = t.url;
  }
